...
The vortex lattice method is essentially an extension of thin airfoil theory into three dimensions.  It could also be thought of as a simplified implementation of a 3D panel code.  All potential theory is based on the assumption of irrotational flow:
```math
\nabla \times \vec{V} = 0
```
...
Solutions are generally built up using singularities that already satisfy Laplace's equation and the boundary condition in the far-field.  In `VortexLattice` we use straight line vortex filaments arranged into rings. Each lifting surface is divided spanwise and chordwise into panels, and each panel has an associated vortex ring with four edges.

Wakes are also modeled in `VortexLattice` using vortex rings.  For steady simulations, a single vortex ring is typically used which starts at the trailing edge of a surface and extends into the farfield. For unsteady simulations, vortex rings are typically shed over time from the trailing edge of each surface and convect with the local velocity field.

The first of Helmholtz' vortex theorems states that vortex strength (and direction) must be constant throughout a vortex ring.  The second theorem states that the vortex must form a closed path or end at the boundary. The vortex rings used in `VortexLattice` satisfy both of Helmhotz' vortex theorems.

The vortex lattice method uses the thin airfoil assumption.  A wing may be modeled using one or more flat or curved surfaces with zero thickness.  To model thickness, 3D panel methods must be used instead.

Because vortex filaments automatically satisfy the governing equations, the only remaining boundary conditions to satisfy are flow tangency and the Kutta condition.  Flow tangency means that the normal component of the velocity must be zero at the surface.  This condition is applied at select control points.  Because each vortex ring has a constant (as yet unknown) strength, we can only have one control point per panel, thus maintaining an equal number of unknowns and equations.

The vortex lattice method uses what is called a lumped vortex method, which means that distributed vorticity associated with each panel is lumped into one vortex.  In `VortexLattice` this lumped vortex is placed at the quarter-chord of each panel.  The strength of this lumped vortex is equal to the net circulation strength on the panel.  For the leading edge panel the lumped vortex strength is equal to the strength of the associated vortex ring.  For all other panels the lumped vortex strength is equal to the strength of the associated vortex ring, minus the strength of the vortex ring associated with the previous chordwise panel.  Each panel's control point is placed at the three quarters-chord.  

To motivate our placement choices, consider modeling an airfoil with just a lumped vortex and a control point.  Because there is only one chordwise control point we cannot resolve flow tangency at multiple points along the camber line, and so it makes sense to impose the camber in the placement of the control point.  A parabolic camber is a reasonable approximation for a general camber line.  This means that the camber line is described by:
```math
...
```math
    \vec{V}_n|_{cp} = 0
```
where the ``cp`` subscript denotes a control point, and the boundary condition is applied at each control point separately.  For steady simulations, the velocity can be broken up into four terms: the freestream velocity, velocity from rigid-body rotation, induced velocity from the surface panel vortex rings, induced velocity from the wake panel vortex rings, and other external velocity sources such as upstream wakes or gusts.  For unsteady simulations, velocity at the control points may also result from surface motion and/or deformation.  The freestream velocity is defined as the negative of the translational motion of the vehicle (thus it is constant for the entire aircraft, unlike gusts which may vary along the aircraft and are lumped in ``\vec V_{other}``). The flow tangency boundary condition is thus:
```math
    \left[(\vec{V}_\infty - \vec{\Omega} \times \vec{r}_b + \vec{V}_{ind,~surf} + \vec{V}_{ind,~wake} + \vec{V}_{def} + \vec{V}_{other}) \cdot \hat{n} \right]_{cp}= 0
```
where ``\vec{r}_b`` is a vector from the aircraft center of gravity to a point of interest.  We will move the induced terms to one side, and all other terms to the other side (where the ``cp`` subscript is dropped for simplicity in notation):
```math
    (\vec{V}_{ind,~surf}+ \vec{V}_{ind,~wake}) \cdot \hat{n} = - (\vec{V}_\infty - \vec{\Omega} \times \vec{r}  + \vec{V}_{def} + \vec{V}_{other}) \cdot \hat{n}
```
To keep the notation simple we will denote the velocity on the right-hand side as ``\vec V_{ext}``.
```math
(\vec{V}_{ind,~surf}+ \vec{V}_{ind,~wake}) \cdot \hat{n} = - \vec{V}_{ext} \cdot \hat{n}
```
In general the rotational velocity, induced velocity, and external velocity all vary along the aircraft, and so this boundary condition (flow tangency) will be applied at each control point ``i``.
```math
(\vec{V}_{ind,~surf,~i}+ \vec{V}_{ind,~wake,~i}) \cdot \hat{n}_i = - \vec{V}_{ext, i} \cdot \hat{n}_i
```

### External Velocities

Angle of attack and sideslip angle are defined in the standard way.  In this case, the the freestream velocity vector in the body axes is:
```math
...
The normal direction is a function of the local twist, dihedral, and camber.  If, however, we only have one chordwise panel, it doesn't make sense to use camber in defining the normal direction (especially since camber was implicitly accounted for in the use of positioning the bound vortex and the control point).  The normal direction then becomes a function of just the local twist and dihedral.

We can resolve ``\hat\zeta`` and ``\hat{n}`` into components as follows:
```math
...
Alternatively, we can determine the normal vector on a given panel from two vectors defined on the same panel.
```math
\hat{n} = \frac{\vec{r_1} \times \vec{r_2}}{||\vec{r_1} \times \vec{r_2}||}
```
This is the approach adopted in `VortexLattice`.
