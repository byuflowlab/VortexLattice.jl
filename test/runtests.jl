...
# @testset "Unsteady Vortex Lattice Method - Wing + Tail" begin
    ...
    ns_multiplier = 1
    nc_multiplier = 1
    time_step = 6.4 # in [6.4, 3.2, 1.6, 0.8, 0.4, 0.2, 0.1, 0.05]
    ...
    # t
    t = range(0.0, 10.0, step=time_step)
    dt = t[2:end] - t[1:end-1]

    println("Running VortexLattice (nsteps = $(length(t)))")
    @time begin
    ...
    # copy to fast-access vector
    fast_access_vector = Vector{VortexLattice.FastMultipolePanel{Float64}}(undef,sum(length.(system.wakes)))

    function update_fmm_panels!(fmm_panels, wake_panels::Vector{Matrix{VortexLattice.WakePanel{TF}}}) where TF
        i_start = 0
        for wake in wake_panels
            for (i,panel) in enumerate(wake)
                fmm_panels[i_start+i] = VortexLattice.FastMultipolePanel(panel)
            end
            i_start += length(wake)
        end
    end
    ...
    @show sum(length.(system.wakes))
    @btime update_fmm_panels!($fast_access_vector, $system.wakes)

# end

# @testset "velocity gradient" begin

# # test velocity gradient of a vortex filament of unit strength
# function test_velocity_gradient(r1,r2)
#     function velocity(x)
#         return VortexLattice.bound_induced_velocity(r1+x, r2+x, false, 1e-2)
#     end
#     return ForwardDiff.jacobian(velocity, zeros(3))
# end

# r1 = SVector{3}(0.21490591034021655,
#         0.5811234218620989,
#         0.8079954052156225)

# r2 = SVector{3}(0.04054862116722213,
#         0.6134119927849612,
#         0.6270695997855336)

# analytic_gradient = VortexLattice.bound_velocity_gradient(r1,r2)
# check = ForwardDiff.jacobian((x)->VortexLattice.bound_induced_velocity(r1+x,r2+x,false,1e-2),zero(SVector{3,Float64}))
# for i in eachindex(check)
#     @show isapprox(analytic_gradient[i], check[i]; atol=1e-12)
# end
