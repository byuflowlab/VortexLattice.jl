...
function _nonlinear_analysis!(system, r, damping, tol, vel, vx, vy, vz, x_c)
    ...
    for i in eachindex(system.surfaces)
        ...
        for j in eachindex(sections)
            ...
            if converged && !iszero(section.airfoil.cl)
                if (abs((section.Γs[3] - section.Γs[1])/section.Γs[1]) > tol)
                    converged = false
                end
            end
...
function update_section_forces!(system, 
                                vel=zeros(eltype(system.Γ),3),
                                vx = zeros(eltype(system.Γ), maximum(size.(system.surfaces,1))), 
                                vy = zeros(eltype(system.Γ), maximum(size.(system.surfaces,1))), 
                                vz = zeros(eltype(system.Γ), maximum(size.(system.surfaces,1))), 
                                x_c = zeros(eltype(system.Γ), maximum(size.(system.surfaces,1))))
    for i in eachindex(system.surfaces)
        isempty(system.sections[i]) && continue # Ensure sections are assigned, if not do not perform nonlinear analysis on this surface
        surface = system.surfaces[i]
        nc = size(surface, 1)
        ...
        for j in eachindex(sections)
            ...
            lift = section.cl[1] * v_mag^2 * RHO * 0.5 * total_chord
            drag = section.cd[1] * v_mag^2 * RHO * 0.5 * total_chord
            section.force[1] = (lift * l_hat + drag * d_hat) ./ RHO ./ system.reference[1].V
        end
    end
    return system
end
